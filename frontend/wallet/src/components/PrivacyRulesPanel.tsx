...
export function PrivacyRulesPanel() {
  ...
  const handleToggle = (mode: PrivacyMode) => {
    if (currentMode === mode) return;        // already active — noop
    execSetMode({
      address: CONTRACTS.PRIVACY_ROUTER,
      abi: PRIVACY_ROUTER_ABI,
  ...
  const isCustom     = currentMode === PrivacyMode.CUSTOM;
  const isPublic     = currentMode === PrivacyMode.PUBLIC;
  const isBusy       = setModePending || setModeConfirming || setRulesPending || setRulesConfirming;

  const toggleClasses = (on: boolean, accent: string) =>
    `relative inline-flex h-7 w-12 items-center rounded-full transition-colors focus:outline-none disabled:opacity-50 ${
      on ? accent : 'bg-gray-600'
    }`;
  ...
  return (
    <div className="glass rounded-2xl p-6 space-y-6">
      ...
      <div className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
        isPublic ? 'bg-white/5 border-white/10' : 'bg-white/[0.02] border-white/5 opacity-60'
      }`}>
        <div>
          <p className="font-medium text-white">Public</p>
          <p className="text-sm text-gray-400 mt-0.5">All incoming transfers go to your public balance</p>
        </div>
        <button
          onClick={() => handleToggle(PrivacyMode.PUBLIC)}
          disabled={isBusy || !address}
          className={toggleClasses(isPublic, 'bg-gray-500')}
      ...
      {/* ── Toggle 2 — Auto-Shield ──────────────────────── */}
      <div className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
        isAutoShield ? 'bg-white/5 border-private-500/30' : 'bg-white/[0.02] border-white/5 opacity-60'
      }`}>
        <div>
          <p className="font-medium text-white">Auto-Shield</p>
          <p className="text-sm text-gray-400 mt-0.5">Automatically shield all incoming ETH into your private sub-account</p>
        </div>
        <button
          onClick={() => handleToggle(PrivacyMode.AUTO_SHIELD)}
          disabled={isBusy || !address}
          className={toggleClasses(isAutoShield, 'bg-private-600')}
      ...
      {/* ── Toggle 3 — Custom Rules ─────────────────────── */}
      <div className={`flex items-center justify-between p-4 rounded-xl border transition-all ${
        isCustom ? 'bg-white/5 border-shadow-500/30' : 'bg-white/[0.02] border-white/5 opacity-60'
      }`}>
        <div>
          <p className="font-medium text-white">Custom Rules</p>
          <p className="text-sm text-gray-400 mt-0.5">Shield based on min amount, token whitelist, and sender whitelist</p>
        </div>
        <button
          onClick={() => handleToggle(PrivacyMode.CUSTOM)}
          disabled={isBusy || !address}
          className={toggleClasses(isCustom, 'bg-shadow-600')}
          aria-label="Toggle custom rules"
        >
          <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow-sm transition-transform ${
      ...
      {/* ── Custom Rules fields — only when CUSTOM ──────── */}
      {isCustom && (
